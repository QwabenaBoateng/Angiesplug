      ...
      <Link to={`/product/${product.id}`}>
        <div className="aspect-[3/4] overflow-hidden relative">
          ...
          <div className="absolute top-4 right-4">
            <span className="text-[10px] font-semibold text-blue-400 uppercase tracking-normal bg-blue-500/10 px-3 py-1 rounded-full border border-blue-500/20 backdrop-blur-md">
              NEW DROP
            </span>
          </div>
        </div>
        <div className="p-6">
          <h3 className="font-semibold text-lg text-slate-900 tracking-tight uppercase truncate mb-1">{product.name}</h3>
          <p className="text-slate-500 text-[10px] uppercase font-semibold tracking-normal mb-4">
    ...
    return (
      <div className="min-h-screen bg-[#f8f8f6] flex flex-col items-center justify-center p-4">
        <div className="text-center max-w-md">
          <h1 className="text-4xl font-semibold text-slate-900 tracking-tight mb-6  uppercase">PRODUCT <span className="text-red-500">OFFLINE</span></h1>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-12 gap-6">
          <Link to="/shop" className="group inline-flex items-center space-x-3 text-slate-600 hover:text-slate-900 transition-colors">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-[0.3em]">Back to catalog</span>
          </Link>
          
          <nav className="glass py-2 px-6 rounded-full border border-slate-200">
            <ol className="flex items-center space-x-3 text-[10px] font-semibold uppercase tracking-normal overflow-x-auto no-scrollbar whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            <div className="relative aspect-[3/4] rounded-[3rem] overflow-hidden border border-slate-200 shadow-2xl group">
              ...
              <div className="absolute top-8 left-8">
                <div className="glass px-6 py-3 rounded-2xl border border-slate-200">
                  <p className="text-[10px] font-semibold text-blue-500 uppercase tracking-[0.4em]">EXQUISITE SELECTION</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col justify-center">
            <div className="mb-10 space-y-4">
              <span className="text-[10px] font-semibold text-blue-500 uppercase tracking-[0.5em] block">In Stock & Verified</span>
              <h1 className="text-5xl md:text-7xl font-semibold text-slate-900 tracking-tight uppercase  leading-none">{product.name}</h1>
              ...
              <div className="flex items-center space-x-6 pt-2">
                ...
                <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-[0.2em]  underline decoration-blue-500/30">124 Elite Reviews</span>
              </div>
            </div>

            <div className="mb-12">
              <div className="flex items-baseline space-x-4">
                <span className="text-5xl font-semibold text-slate-900 tracking-tight ">₵{product.price}</span>
                <span className="text-slate-600 font-bold uppercase tracking-normal text-[10px] line-through">₵{Math.round(product.price * 1.3)}</span>
              </div>
            </div>

            <div className="glass-card rounded-[2.5rem] p-8 sm:p-10 border-slate-200 mb-10">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-[0.3em] mb-4">Product Details</h3>
            ...
            <div className="space-y-10 mb-12">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                  <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-normal">Select Fit</h3>
                  <button className="text-[10px] font-semibold text-blue-500 uppercase tracking-normal hover:text-slate-900 transition-colors">Size Guide</button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {['XS', 'S', 'M', 'L', 'XL', 'XXL'].map((size) => (
                    <button
                      ...
                      className={`min-w-[4rem] h-14 rounded-2xl border-2 font-semibold text-xs transition-all flex items-center justify-center ${
                        selectedSize === size
              ...
              {/* Color Select */}
              <div className="space-y-4">
                <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-normal px-2">Colorway</h3>
                <div className="flex flex-wrap gap-3">
                  {['STELTH BLACK', 'ARCTIC WHITE', 'NEON CRIMSON', 'CYBER BLUE'].map((color) => (
                    <button
                      key={color}
                      onClick={() => setSelectedColor(color)}
                      className={`px-6 h-12 rounded-2xl border-2 font-semibold text-[10px] transition-all uppercase tracking-normal ${
                        selectedColor === color
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div className="space-y-4 w-full sm:w-auto">
                   <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-normal px-2">Batch Size</h3>
                  <div className="flex items-center bg-white rounded-[1.5rem] border border-slate-200 p-1 h-14 w-full sm:w-40">
                    ...
                    <span className="w-10 text-center font-semibold text-slate-900 text-xl tabular-nums ">
                      {quantity}
                    </span>
                    <button
                ...
                <div className="flex-1 w-full space-y-4">
                   <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-normal px-2 invisible sm:visible">Action</h3>
                  <button
                    onClick={handleAddToCart}
                    disabled={isAddingToCart || !selectedSize}
                    className="btn-gradient w-full py-5 text-sm font-semibold tracking-[0.2em] shadow-2xl shadow-blue-500/20 active:scale-[0.98] disabled:opacity-30 disabled:cursor-not-allowed group h-14"
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 py-10 border-t border-slate-200">
              <div className="flex items-center space-x-4 group cursor-default">
                ...
                <span className="text-[10px] font-semibold text-slate-600 uppercase tracking-normal">Global Express <br /> <span className="text-slate-900">Shipping</span></span>
              </div>
              <div className="flex items-center space-x-4 group cursor-default">
                <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition-all">
                  <Shield size={18} />
                </div>
                <span className="text-[10px] font-semibold text-slate-600 uppercase tracking-normal">Secure <br /> <span className="text-slate-900">Payments</span></span>
              </div>
              <div className="flex items-center space-x-4 group cursor-default">
                <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition-all">
                  <RotateCcw size={18} />
                </div>
                <span className="text-[10px] font-semibold text-slate-600 uppercase tracking-normal">30-Day Return <br /> <span className="text-slate-900">Guarantee</span></span>
              </div>
            </div>
          </div>
        </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-32 pt-24 border-t border-slate-200">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-16 gap-6">
              <div>
                <span className="text-[10px] font-semibold text-blue-500 uppercase tracking-[0.4em] mb-4 block">Recommended Drops</span>
                <h2 className="text-4xl md:text-6xl font-semibold text-slate-900 tracking-tight uppercase  ">RELATED <span className="text-gradient">GEAR</span></h2>
              </div>
              <Link to="/shop" className="btn-glass px-8 py-3 text-[10px] font-semibold tracking-normal uppercase">View All Products</Link>
            </div>
