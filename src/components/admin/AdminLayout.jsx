...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f6] text-slate-700 selection:bg-blue-500/30 font-sans">
      {/* Mobile sidebar */}
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        <div className="fixed inset-0 bg-[#f8f8f6]/80 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />
              ...
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={() => setSidebarOpen(false)}
                  >
                    <Icon className={`mr-4 h-5 w-5 ${active ? 'text-blue-500' : 'text-slate-600 group-hover:text-slate-600'}`} />
                    {item.name}
                  </Link>
                )
              })}
      ...
      <div className="hidden lg:flex lg:flex-shrink-0 fixed inset-y-0 z-20">
        <div className="flex flex-col w-72">
          <div className="flex flex-col h-full bg-[#f8f8f6]/50 backdrop-blur-xl border-r border-slate-200">
              ...
              <nav className="flex-1 px-4 space-y-2">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`mr-4 h-5 w-5 transition-colors ${active ? 'text-blue-500' : 'text-slate-600 group-hover:text-slate-600'}`} />
                      {item.name}
                    </Link>
                  )
                })}
      ...
      <div className="lg:pl-72 flex flex-col flex-1 min-h-screen">
        {/* Top bar */}
        <div className="sticky top-0 z-10 bg-[#f8f8f6]/70 backdrop-blur-xl border-b border-slate-200 transition-all">
          <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center">
              <button
                type="button"
                className="lg:hidden -ml-2 mr-4 p-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-blue-500"
